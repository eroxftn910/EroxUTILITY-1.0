# Fortnite Optimization Tool - Fixed for EroxUTILITY
...
$PerfKey = "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\FortniteClient-Win64-Shipping.exe\PerfOptions"
$QoSKey = "HKLM:\SOFTWARE\Policies\Microsoft\Windows\QoS\FortniteClient-Win64-Shipping"
...
function Get-FortniteExe {
    $paths = @(
        ...
        "${env:ProgramFiles(x86)}\Epic Games\Fortnite\FortniteGame\Binaries\Win64\FortniteClient-Win64-Shipping.exe"
    )

    foreach ($drive in Get-PSDrive -PSProvider FileSystem) {
        $paths += "$($drive.Root)Program Files\Epic Games\Fortnite\FortniteGame\Binaries\Win64\FortniteClient-Win64-Shipping.exe"
        $paths += "$($drive.Root)Program Files (x86)\Epic Games\Fortnite\FortniteGame\Binaries\Win64\FortniteClient-Win64-Shipping.exe"
    }
...
function Apply-QoS {
    try {
        $exe = Get-FortniteExe
        if (-not $exe) {
            Log "Fortnite executable introuvable." "Red"
            return
        }
        ...
        $values = @{
            "Application Name" = "FortniteClient-Win64-Shipping.exe"
            "AppPathName" = $exe
            "DSCP Value" = "46"
            "Local IP" = "*"
...
function Apply-GPU {
    try {
        ...
        Log "GPU High Performance activé pour Fortnite." "Green"
    }
    catch {
...
function Apply-RunAsAdmin {
    try {
        ...
        $ifeo = "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\FortniteClient-Win64-Shipping.exe"
        New-Item -Path $ifeo -Force | Out-Null
        New-ItemProperty -Path $ifeo -Name "RunAsAdmin" -Value 1 -PropertyType DWord -Force | Out-Null

        Log "Run as administrator activé pour Fortnite." "Green"
    }
    catch {
...
function Apply-Firewall {
    try {
        ...
        New-NetFirewallRule -DisplayName "FortniteClient In" -Direction Inbound -Program $exe -Action Allow -ErrorAction SilentlyContinue | Out-Null
        New-NetFirewallRule -DisplayName "FortniteClient Out" -Direction Outbound -Program $exe -Action Allow -ErrorAction SilentlyContinue | Out-Null
...
function Apply-Defender {
    try {
        $dir = Get-FortniteDir
        if (-not $dir) {
            Log "Dossier Fortnite introuvable." "Red"
            return
        }
...
function Remove-All {
    try {
        ...
        Remove-NetFirewallRule -DisplayName "FortniteClient In" -ErrorAction SilentlyContinue
        Remove-NetFirewallRule -DisplayName "FortniteClient Out" -ErrorAction SilentlyContinue
...
$form.Text = "EroxUTILITY - Fortnite Optimization"
...
$title.Text = "Fortnite Optimization Toolkit"
...
$subtitle.Text = "Made for EroxUTILITY - Optimizes Fortnite performance for Windows."
...
$items = @(
    ...
    @("Run As Admin", "Make Fortnite run as administrator."),
    @("Firewall Rules", "Add Inbound and Outbound firewall rules."),
    @("Defender Exclusion", "Exclude Fortnite from Defender scans."),
